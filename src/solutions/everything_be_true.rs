...
impl Type {
    fn name(&self) -> &str {
        match self {
            Type::Person(person) => &person.name,
    ...
    fn role(&self) -> &str {
        match self {
            Type::Person(person) => &person.role,
    ...
    fn is_bot(&self) -> bool {
        match self {
            Type::Person(person) => person.is_bot,
            _ => panic!("Not a valid type"),
        }
    }
    ...
    fn number(&self) -> i32 {
        match self {
            Type::Pokemon(pokemon) => pokemon.number,
            _ => panic!("Not a valid type"),
        }
    }
    ...
    fn caught(&self) -> i32 {
        match self {
            Type::Pokemon(pokemon) => pokemon.caught,
            _ => panic!("Not a valid type"),
        }
    }
}
...
pub fn truth_check(collection: Vec<Type>, pre: &str) -> bool {
